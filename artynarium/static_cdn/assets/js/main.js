...
$(document).ready(function () {
	...
	$(".reply-review").hide();
	$(".btn-reply button").click(function () {
		$(this).parent().next().toggle();
	});

	$('#side').stick_in_parent();
	'use strict';
	$('span.sss > input:checked').parent().addClass('asd');
	//Blog
	quantityInputs();
	// Owl Carousel
	$('#owl-1').owlCarousel({
	...
	$('#checkout-discount-input')
		.on('focus', function () {
			// Hide label on focus
			$(this).parent('div').find('label').css('opacity', 0);
		})
		.on('blur', function () {
			...
			if ($this.val().length !== 0) {
				...
				$this.parent('div').find('label').css('opacity', 1);
			}
		});
